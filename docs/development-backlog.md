...
> Comprehensive test suite: local kind environment, Helm validation, unit/integration/contract tests, E2E, and realistic user journey tests.

### 11.1 — Kind Environment & Helm Validation

- [ ] **T-11.1.1** — Kind cluster configuration  
  **Size:** S  
  Create `tests/kind/cluster.yaml` (K8s 1.30, 3 nodes, port mappings 80/443/8080/8081/3000/3001/9090, local-path-provisioner). Makefile targets: `kind-up`, `kind-down`, `kind-reset`.  
  **Acceptance:** `make kind-up` creates a working cluster in under 2min. `kubectl get nodes` shows 3 nodes Ready.

- [ ] **T-11.1.2** — Image build and load pipeline  
  **Size:** M  
  `docker-bake.hcl` defining all service images. `make kind-build` parallelizes builds. `make kind-load` loads images into kind with incremental detection (only changed images reloaded via digest comparison).  
  **Acceptance:** Full build + load completes in < 5min. Incremental reload takes < 30s when no changes.

- [ ] **T-11.1.3** — Seed data fixtures  
  **Size:** L  
  Create `tests/fixtures/` with: 5 test users (one per tier), 1000 synthetic listings (ES/IT/FR/PT/GB), zone polygons for 10 major cities, minimal pre-trained ONNX models, 10 alert rules, sample AI conversations. `tests/fixtures/load.py` bulk loader.  
  **Acceptance:** `make kind-seed` loads all fixtures in < 30s. Queries return expected data.

- [ ] **T-11.1.4** — Test mode flag implementation  
  **Size:** M → T-11.1.3  
  Services read `ESTATEGAP_TEST_MODE=true` env var and: use FixtureSpider for scraping, accept test Stripe webhook signatures, use FakeLLMProvider for AI chat, accept `NOW_OVERRIDE` for time travel, accelerate cron schedules.  
  **Acceptance:** Tests using test mode produce deterministic results. No real external API calls.

- [ ] **T-11.1.5** — Helm lint and schema validation  
  **Size:** S  
  `helm/estategap/values.schema.json` covers all top-level values. `helm lint` passes on all 4 values profiles. CI job runs on every PR.  
  **Acceptance:** Schema validates all values profiles. Invalid values rejected by helm install.

- [ ] **T-11.1.6** — helm-unittest suite  
  **Size:** L  
  Install helm-unittest plugin. Create 20+ test cases in `helm/estategap/tests/` covering: deployment rendering per profile, HPA conditional, ingress paths, network policies, sealed secrets, feature flag toggles.  
  **Acceptance:** `helm unittest ./helm/estategap` passes all tests. Test coverage includes all template logic.

- [ ] **T-11.1.7** — Installation test on kind  
  **Size:** M → T-11.1.1, T-11.1.2  
  Bash script `tests/helm/install-test.sh` that installs chart on kind, waits for all Deployments ready (3min timeout), verifies all `/readyz` endpoints respond, checks no CrashLoopBackOff. Runs in CI.  
  **Acceptance:** Installation test passes on clean kind cluster. Detects common deployment regressions.

- [ ] **T-11.1.8** — Upgrade and rollback tests  
  **Size:** M → T-11.1.7  
  Script that installs chart v0.1.0, seeds data, snapshots DB, upgrades to v0.2.0, verifies no data loss, rolls back, verifies service still works.  
  **Acceptance:** Rolling upgrade succeeds without downtime (zero 5xx during rollout). Rollback restores state.

- [ ] **T-11.1.9** — Chart conformance checks  
  **Size:** M → T-11.1.5  
  Python script `tests/helm/conformance.py` validates: all resources have standard labels, namespace, resource limits, securityContext, probes. No `:latest` tags. NetworkPolicies enforce isolation.  
  **Acceptance:** All resources pass conformance. Violations fail CI.

- [ ] **T-11.1.10** — Port forwarding and dev utilities  
  **Size:** S  
  Script `tests/kind/port-forward.sh` auto-forwards service ports. `make kind-logs` tails pod logs (via `stern`). `make kind-shell SERVICE=x` opens shell.  
  **Acceptance:** `curl localhost:8080/healthz` works after deploy. All dev utilities documented in `docs/development.md`.

### 11.2 — Unit, Integration & Contract Tests

- [ ] **T-11.2.1** — Go test infrastructure  
  **Size:** M  
  `golangci-lint` config at `.golangci.yml` (strict rules). Table-driven test pattern documented. Shared `libs/testhelpers/` with testcontainers helpers (Postgres, Redis, NATS, MinIO). CI enforces 80% coverage.  
  **Acceptance:** `go test -race -cover ./...` passes for all Go services. Coverage ≥ 80%.

- [ ] **T-11.2.2** — Python test infrastructure  
  **Size:** M  
  pytest config per service, `libs/common/testing/` with fixtures for containers, `conftest.py` patterns. `ruff` + `mypy --strict` in CI. Coverage ≥ 80%.  
  **Acceptance:** `pytest` passes for all Python services. Linters pass. Coverage met.

- [ ] **T-11.2.3** — Frontend test infrastructure  
  **Size:** M  
  Vitest + React Testing Library + MSW setup. `eslint` strict. `tsc --noEmit` passes. Coverage ≥ 70%.  
  **Acceptance:** `pnpm test` passes. Component tests for all shared components exist.

- [ ] **T-11.2.4** — Database migration tests  
  **Size:** M → T-1.1  
  Tests in `services/pipeline/tests/integration/test_migrations.py`: upgrade from scratch, downgrade each migration, idempotency, no data loss on existing data.  
  **Acceptance:** All migrations reversible. No migration deletes data unintentionally.

- [ ] **T-11.2.5** — gRPC integration tests  
  **Size:** L  
  Each gRPC service (ai-chat, ml-scorer, proxy-manager) has integration tests that start real server in-process, call via client stubs, test streaming RPCs, verify error handling.  
  **Acceptance:** All RPCs have ≥ 1 integration test. Streaming RPCs test cancellation and timeouts.

- [ ] **T-11.2.6** — NATS integration tests  
  **Size:** M  
  Each NATS consumer has tests: publish message, wait for processing, verify side effects. Test ack/nak and redelivery behavior.  
  **Acceptance:** All consumers verified. Message loss impossible (JetStream ack semantics tested).

- [ ] **T-11.2.7** — Data pipeline integration test  
  **Size:** L → T-11.2.6  
  End-to-end test: publish raw listing to NATS → verify it flows through normalizer → dedup → enricher → scorer → DB within 30s. Batch test with 100 listings.  
  **Acceptance:** Pipeline latency < 30s per listing. Batch throughput ≥ 50 listings/min in test env.

- [ ] **T-11.2.8** — Protobuf contract tests  
  **Size:** M  
  `buf breaking --against '.git#branch=main'` in CI. Contract tests verify client/server agree on message format. Breaking changes require explicit approval.  
  **Acceptance:** CI blocks breaking proto changes. All RPCs have contract tests.

- [ ] **T-11.2.9** — OpenAPI contract tests  
  **Size:** M  
  Frontend types auto-generated from OpenAPI spec. E2E tests validate API responses against schema. Consumer-driven fixtures in `tests/contracts/frontend/`.  
  **Acceptance:** Type drift between frontend and API impossible. Contract violations fail CI.

### 11.3 — E2E Test Suite (API + Playwright)

- [ ] **T-11.3.1** — REST API test suite  
  **Size:** XL → T-11.1.7  
  Python + pytest + httpx tests for every documented endpoint. Auth flows, rate limiting per tier, pagination, all filters, error responses, currency conversion, subscription gating.  
  **Acceptance:** All endpoints in OpenAPI spec have ≥ 1 happy path test + 3 error tests. Runs in < 10min.

- [ ] **T-11.3.2** — WebSocket test suite  
  **Size:** L → T-11.1.7  
  Python `websockets` client tests: JWT auth, chat protocol (all message types), real-time deal alerts, reconnection with session restore, ping/pong, idle timeout. Concurrent connections (100).  
  **Acceptance:** All WebSocket message types tested. Concurrency test passes with 100 sessions.

- [ ] **T-11.3.3** — Playwright browser tests  
  **Size:** XL → T-11.1.7  
  Playwright TypeScript tests covering 10+ flows: auth, AI chat, search, listing detail, dashboard, map, alerts, subscription, admin, responsive. Page Object Model pattern.  
  **Acceptance:** 10+ flows pass on Chromium, Firefox, WebKit. All elements have `data-testid`.

- [ ] **T-11.3.4** — Visual regression tests  
  **Size:** M → T-11.3.3  
  Playwright `toHaveScreenshot()` baselines for key pages. Tolerance 1%. Update workflow documented.  
  **Acceptance:** 10+ visual baselines stored. False positive rate < 1% over 50 runs.

- [ ] **T-11.3.5** — Concurrency tests  
  **Size:** M → T-11.3.1  
  Multi-user scenarios: 100 concurrent chat sessions, simultaneous alert creation, concurrent portfolio updates.  
  **Acceptance:** 100 concurrent users without errors. No cross-user data leakage.

- [ ] **T-11.3.6** — CI E2E workflow  
  **Size:** M → T-11.3.1 through T-11.3.5  
  `.github/workflows/ci-e2e.yml` with kind-action, sharding across 4 runners, artifact upload on failure (logs, screenshots, video, DB dump).  
  **Acceptance:** Full E2E suite runs in CI in < 20min. Failures produce complete artifacts.

### 11.4 — Use Case / User Journey Tests

- [ ] **T-11.4.1** — Test infrastructure for journeys  
  **Size:** M → T-11.3.1, T-11.3.3  
  Python pytest setup with helpers: ApiClient, WSClient, Playwright wrapper, ClusterHelper (kubectl), DbClient, TimeTravel (NOW_OVERRIDE), EmailSpy, TelegramSpy. TestContext base class with auto artifact collection on failure.  
  **Acceptance:** Framework supports all 15 journey tests. Shared helpers documented.

- [ ] **T-11.4.2** — UJ-01 to UJ-05 (onboarding, search, alerts, AI, subscription)  
  **Size:** XL → T-11.4.1  
  Implement first 5 user journey tests. Each with Given/When/Then structure, latency assertions, failure artifacts.  
  **Acceptance:** All 5 journeys pass on kind. Each runs in < 5min.

- [ ] **T-11.4.3** — UJ-06 to UJ-10 (portfolio, admin retrain, free delay, multi-country, GDPR)  
  **Size:** XL → T-11.4.1  
  Implement next 5 journeys.  
  **Acceptance:** All 5 journeys pass. Admin retrain test completes in < 5min (minimal training data).

- [ ] **T-11.4.4** — UJ-11 to UJ-15 (price drop, scraping recovery, i18n, WS reconnect, latency)  
  **Size:** XL → T-11.4.1  
  Implement final 5 journeys. Includes end-to-end latency test (scrape-to-alert < 2min in test mode).  
  **Acceptance:** All 5 journeys pass. Latency assertions met.

- [ ] **T-11.4.5** — Performance baseline and regression tracking  
  **Size:** M → T-11.4.2 through T-11.4.4  
  Record latency per UJ test in tracking file. CI compares current run vs baseline. Regressions > 20% fail build.  
  **Acceptance:** Baselines established for all 15 journeys. Regression detection works.

- [ ] **T-11.4.6** — Test documentation  
  **Size:** M → T-11.4.2 through T-11.4.4  
  `docs/test-scenarios.md` with Given/When/Then for each UJ. `docs/testing-guide.md` with: how to run tests, debug failures, add new journeys.  
  **Acceptance:** Documentation reviewed by team. New developer can run full test suite following docs.
...
| **11 — Testing & QA** | **23** | **~4 weeks** |
| **Total** | **127 tasks** | **~36 weeks (part-time)** |
